...
const defaultConfig = {
  ...
  range_mod: 1.5,
  alpha_mod: 0.7,
  rotateButton: { x: 0.95, y: 0.05, w: 0.01, h: 0.01 },
  switchButton: { x: 0.95, y: 0.95, w: 0.01, h: 0.01 },
  consoleAspectRatio: 3 / 2,
  targetScreenSize: { w: 1242, h: 2688, notch: 44 },
}
...
class RetroDelta {
  ...
  #fitToScreen(layout, orientation) {
    ...
    if (!screens.length) {
      screens.push(this.#addScreen(isPortrait, translucent, target, newSize));
    }
    // logger.debug(screens);
    layout.screens = screens;
  }

  #addScreen(isPortrait, translucent, target, newSize) {
    const screenSize = {
      w: target.w,
      h: target.h,
    };
    ...
    if (!translucent) {
      ...
      if (isPortrait) {
        screenSize.h -= target.notch;
        y += target.notch;
      }
    }
    const h = screenSize.h;
    const aspect = this.config.consoleAspectRatio;
  ...
  #getBorderAsset(layout) {
    ...
    if (asset.endsWith('.pdf')) {
      const { newSize, extent } = layout;
      const gravity = layout.translucent ? 'center' : 'south';
      return this.#getPdfAsPngAsset(asset, newSize, { src, extent, gravity });
    }
