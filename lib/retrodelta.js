...
class RetroDelta {
  ...
  parseDelta(deltaInfo) {
    ...
    if (!representations) {
      return logger.error('Currently only support iphone edgeToEdge');
    }
    if (deltaInfo?.gameTypeIdentifier === 'com.rileytestut.delta.game.ds') {
      return logger.error('Currently does not support NDS');
    }
    this.overlays = [];
    this.screens = {};
    ...
    for (const orientation in representations) {
      ...
      if (layout.screens?.length !== 1) {
        return logger.error('Currently only support single screen');
      }
  ...
  #convertInputToDesc(inputs, frame, extendedEdges, size) {
    ...
    const box = this.#convertBox(frame, extendedEdges, size);
    box.shape = 'rect';
    const desc = {
